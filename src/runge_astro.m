function f = runge_astro(t, X, I_neuro, sum_Ca, sum_IP3, Iwm, Isr)
    ...
    f(1)   = Ier - Ipump + Ileak + Iin - Iout + params.dCa .* sum_Ca ...
        - params.zeta_WM*double(Iwm) - params.zeta_SR*double(Isr); % Calcium
    f(2)   = (h - X(2)) / Tn;                                                % H
    f(3)   = (IP3s - X(3)) * Tr + Iplc + I_neuro + params.dIP3 .* sum_IP3;    % IP3
    f      = f';
end